...
router.get('/asignaturas', verifyToken, (req, res) => { 
    ...
    mysqlConnection.query(`Call SP_moduloAcademico('MA_ASIGNATURAS','SA','1','1','1','null','1')`, (err, rows) => {
        if (!err) {
...
router.get("/asignaturas/:COD_ASIGNATURA", verifyToken, (req, res) => {
    try {
        ...
        const sql = `Call SP_moduloAcademico('MA_ASIGNATURAS','SO','${COD_ASIGNATURA}','1','1','null','1')`;
...
router.post("/asignaturas", verifyToken, (req, res) => {
    try {
        ...
        const sql = `Call SP_moduloAcademico('MA_ASIGNATURAS','I','1','1','1','${NOMBRE_ASIGNATURA}','${Estado_registro}')`;
...
router.put("/asignaturas/:COD_ASIGNATURA", verifyToken,(req, res) => {
    try {
        const { COD_ASIGNATURA } = req.params;
        const { NOMBRE_ASIGNATURA,Estado_registro} = req.body;
        const sql = `Call SP_moduloAcademico('MA_ASIGNATURAS','U','${COD_ASIGNATURA}','1','1','${NOMBRE_ASIGNATURA}','${Estado_registro}')`;
        mysqlConnection.query(sql, error => {
            if (!error) {
                res.json({
...
router.get('/jornadas', verifyToken, (req, res) => { 
    mysqlConnection.query(`Call SP_moduloAcademico('MA_JORNADA','SA','1','1','1','null','1')`, (err, rows) => {
        if (!err) {
            res.status(200).json(rows[0]);
        } else {
            console.log('Se ha producido un error');
...
router.get("/jornadas/:COD_JORNADA", verifyToken, (req, res) => {
    try {
        ...
        const sql = `Call SP_moduloAcademico('MA_JORNADA','SO','${COD_JORNADA}','1','1','null','1')`;
        mysqlConnection.query(sql, (error, results) => {
            if (error) throw error;
            if (results.length > 0) {
                res.status(200).json(results[0]);
...
router.post("/jornadas", verifyToken, (req, res) => {
    try {
        ...
        const sql = `Call SP_moduloAcademico('MA_JORNADA','I','1','1','1','${DESCRIPCION_JOR}','${Estado_registro}')`;
        mysqlConnection.query(sql, error => {
            if (!error) {
                res.json({
   ...
   router.put("/jornadas/:COD_JORNADA", verifyToken, (req, res) => {
    try {
        const { COD_JORNADA } = req.params;
        const { DESCRIPCION_JOR,Estado_registro } = req.body;
        const sql = `Call SP_moduloAcademico('MA_JORNADA','U','${COD_JORNADA}','1','1','${DESCRIPCION_JOR}','${Estado_registro}')`;
        mysqlConnection.query(sql, error => {
            if (!error) {
                res.json({
...
router.get('/anio_academico', verifyToken, (req, res) => {
    ...
    mysqlConnection.query(`Call SP_moduloAcademico('MA_ANIO_ACADEMICO','SA','1','1','1','null','1')`, (err, rows) => {
      if (!err) {
        res.status(200).json(rows[0]);
      } else {
        console.log('Se ha producido un error');
...
//GET por codigo
  router.get('/anio_academico/:COD_ANIO_ACADEMICO', verifyToken,  (req, res) => {
    ...
    const query = `
      Call SP_moduloAcademico('MA_ANIO_ACADEMICO','SO',?,'1','1','null','1');
    `;
...
router.post("/anio_academico", verifyToken, (req, res) => {
    ...
    try {
      ...
      const sql = `Call SP_moduloAcademico('MA_ANIO_ACADEMICO','I','1','1','1','${descripcion}','${Estado_registro}')`;
      mysqlConnection.query(sql, (error) => {
        if (!error) {
          res.json({
    ...
    try {
      //  jwt.verify(req.token, global.secretTokenAccess, (err) => {
           // if (err) {
            //    res.sendStatus(403);
            //} else {
                mysqlConnection.query(`Call SP_moduloAcademico('MA_NIVEL_ACADEMICO','SA','1','1','1','null','1')`, (error, rows) => {
                    if (!error) {
                        res.status(200).json(rows[0]);
                    } else {
                        console.log('Se ha producido un error');
    ...
    try {
        jwt.verify(req.token, global.secretTokenAccess, (err) => {
            if (err) {
                ...
                const sql = `Call SP_moduloAcademico('MA_NIVEL_ACADEMICO','SO','${COD_NIVEL_ACADEMICO}','1','1','null','1')`;
                mysqlConnection.query(sql, (error, results) => {
                    if (error) throw error;
                    if (results.length > 0) {
                        res.status(200).json(results[0]);
    ...
    try {
        jwt.verify(req.token, global.secretTokenAccess, (err) => {
            if (err) {
                ...
                const { descripcion,Estado_registro } = req.body;
                const sql = `Call SP_moduloAcademico('MA_NIVEL_ACADEMICO','I','1','1','1','${descripcion}','${Estado_registro}')`;
                mysqlConnection.query(sql, error => {
                    if (!error) {
                        res.json({
    ...
    try {
        jwt.verify(req.token, global.secretTokenAccess, (err) => {
            if (err) {
                ...
                const { COD_NIVEL_ACADEMICO } = req.params;
                const { descripcion,Estado_registro } = req.body;
                const sql = `Call SP_moduloAcademico('MA_NIVEL_ACADEMICO','U','${COD_NIVEL_ACADEMICO}','1','1','${descripcion}','${Estado_registro}')`;
                mysqlConnection.query(sql, error => {
                    if (!error) {
                        res.json({
...
router.get('/Secciones', verifyToken, (req, res) => {
    try {
      //  jwt.verify(req.token, global.secretTokenAccess, (err) => {
          ...
          //      res.sendStatus(403);
           // } else {
                mysqlConnection.query(`Call SP_moduloAcademico('MA_SECCIONES','SA','1','1','1','null','1')`, (err, rows) => {
                    if (!err) {
                        res.status(200).json(rows[0]);
                    } else {
                        console.log('Se ha producido un error');
    ...
    try {
        jwt.verify(req.token, global.secretTokenAccess, (err) => {
            if (err) {
                ...
                const sql = `Call SP_moduloAcademico('MA_SECCIONES','SO','${COD_SECCIONES}','1','1','null','1')`;
                mysqlConnection.query(sql, (error, results) => {
                    if (error) throw error;
                    if (results.length > 0) {
                        res.status(200).json(results[0]);
    ...
    try {
        jwt.verify(req.token, global.secretTokenAccess, (err) => {
            if (err) {
                ...
                const sql = `Call SP_moduloAcademico('MA_SECCIONES','I','1','1','1','${DESCRIPCION_SECCIONES}','${Estado_registro}')`;
                mysqlConnection.query(sql, error => {
                    if (!error) {
                        res.json({
    ...
    try {
        jwt.verify(req.token, global.secretTokenAccess, (err) => {
            if (err) {
                ...
                const { COD_SECCIONES } = req.params;
                const { DESCRIPCION_SECCIONES,Estado_registro } = req.body;
                const sql = `Call SP_moduloAcademico('MA_SECCIONES','U','${COD_SECCIONES}','1','1','${DESCRIPCION_SECCIONES}','${Estado_registro}')`;
                mysqlConnection.query(sql, error => {
                    if (!error) {
                        res.json({
