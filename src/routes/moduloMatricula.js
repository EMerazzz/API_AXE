...
router.post("/matricula", verifyToken, (req, res) => {
    ...
    try {
      ...
      const sql = `CALL SP_MODULOMATRICULA('MM_MATRICULA', 'I',          0,         ${COD_ESTUDIANTE},  ${COD_NIVEL_ACADEMICO},   ${COD_ANIO_ACADEMICO},    ${COD_SECCIONES},   '${JORNADA}',    'PARAMETROV2',     'PARAMETROV3', '${USUARIO_MODIFICADOR}',    ${Estado_registro});`;
...
router.put("/matricula/:COD_MATRICULA", verifyToken, (req, res) => {
    ...
    try {
      const { COD_MATRICULA } = req.params;
      const { COD_ESTUDIANTE, COD_NIVEL_ACADEMICO, COD_ANIO_ACADEMICO, COD_SECCIONES, JORNADA, USUARIO_MODIFICADOR, Estado_registro} = req.body;
      const sql = `CALL SP_MODULOMATRICULA('MM_MATRICULA', 'U',  ${COD_MATRICULA},         ${COD_ESTUDIANTE},  ${COD_NIVEL_ACADEMICO},   ${COD_ANIO_ACADEMICO},    ${COD_SECCIONES},   '${JORNADA}',    'PARAMETROV2',     'PARAMETROV3', '${USUARIO_MODIFICADOR}',    ${Estado_registro});`;
      mysqlConnection.query(sql, error => {
        if (!error) {
          res.json({
